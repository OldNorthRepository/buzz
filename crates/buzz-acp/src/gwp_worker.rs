...
pub async fn run(mut config: Config) -> anyhow::Result<()> {
    ...
    let ctx = Arc::new(PromptContext {
        ...
        // Same standing-context assembly as the harness: the (custom or
        // built-in) base prompt plus the configured session model.
        ...
        match frame.get("type").and_then(Value::as_str) {
            Some("turn") => {
                ...
                let reply = match &result.outcome {
                    ...
                    // No prompt reached the agent (project authority could
                    // not be established from relay state); the agent is
                    // healthy and the turn is safe to retry.
                    PromptOutcome::ProjectContextIndeterminate(reason) => json!({
                        "type":"error","turn_id":turn_id,
                        "message":format!("project context indeterminate: {reason}"),
                        "retryable":true,
                    }),
