...
impl GatewayExecutor {
    async fn run_turn_on(
        ...
        let request = SubmitJobRequest {
            ...
            agent_id: None,
            // Write routes bind the channel UUID as their gateway context.
            context_id: write_capable.then(|| scope.channel_id().to_string()),
...
impl TurnExecutor for GatewayExecutor {
    async fn run_turn(
        ...
        let (route, write_capable) = self.route_for(scope.channel_id(), None).await;
        self.run_turn_on(&route, write_capable, scope, turn_id, payload, timeout_ms)
            .await
    }
...
pub(crate) fn encode_turn_payload(batch: &FlushBatch) -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        // Optional (absent = the channel's conversation scope), so a payload
        // from a harness that predates session scoping still decodes.
        "thread_root": batch.scope.root_event_id(),
...
pub(crate) fn decode_turn_payload(task: &str) -> Result<FlushBatch, String> {
    ...
    // The harness resolved the session scope once, at admission; carry it
    // across the hop instead of re-deriving it from the last event.
    let scope = match value.get("thread_root") {
        None | Some(serde_json::Value::Null) => SessionScope::Conversation { channel_id },
        Some(serde_json::Value::String(root)) if !root.is_empty() => SessionScope::Thread {
            channel_id,
            root_event_id: root.to_ascii_lowercase(),
        },
        Some(other) => return Err(format!("bad thread_root {other}")),
    };
    Ok(FlushBatch {
        channel_id,
        scope,
...
mod tests {
    ...
    #[test]
    fn thread_scope_survives_the_hop() {
        let channel = Uuid::new_v4();
